...
export function UsernameSelector({ query, events, onSelect, context, localLinks = false }) {
    ...
    <li className={cn(style.item, isCurrent && style.itemCurrent)} onClick={clk}>
      ...
      <span className={style.itemText}>
        ...
        {account.username !== account.screenName && (
          <span className={style.screenName}>{withCustomEmojis(account.screenName)}</span>
        )}
      </span>
      {localLink ? (
